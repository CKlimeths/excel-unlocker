...
function handleDragAndDrop() {
    ...
    if (
        possibleFile &&
        !possibleFile.startsWith("-") &&
        possibleFile.endsWith(".xlsx")
    ) {
            ...
            processExcelFile(filePath, undefined, "detect")
                .then((result) => {
                    ...
                    if (result.protected) {
                        ...
                        console.log(
                            `   原文件状态: 未受保护 → 未执行操作（使用 add 命令添加保护）`,
                        )
                    }
                })
                .catch((error) => {
                    ...
                    console.log("\n💡 使用以下命令手动处理:")
                    console.log(
                        "   excel-unlocker remove --input <file>  # 移除保护",
                    )
                    console.log(
                        "   excel-unlocker add --input <file>     # 添加保护",
                    )
                    process.exit(1)
                })
            return
