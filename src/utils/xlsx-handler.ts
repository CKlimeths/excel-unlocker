 ...
 * 检测工作表是否受保护
 */
async function isWorksheetProtected(worksheet: ExcelJS.Worksheet): Promise<boolean> {
  try {
    // 检查工作表保护状态
 ...
 * 移除工作表保护
 */
async function removeWorksheetProtection(worksheet: ExcelJS.Worksheet): Promise<void> {
  try {
 ...
 * 添加工作表保护
 */
async function addWorksheetProtection(worksheet: ExcelJS.Worksheet, password?: string): Promise<void> {
  try {
...
export async function processExcelFile(
  ...
  // 如果没有工作表被处理
  if (operation === 'remove' && !anyProtected && operationPerformed === 'none') {
